...
<section class="content">
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <center>
                <div class="container-fluid">
                    <form method="POST" action="<?php echo base_url(); ?>admin/search_prelim_dlist">
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                ...
                <div class="card">
                    <div class="header">
                        <!-- <?php foreach($search_prelim_dlist as $row){ 
                            $sy = $row['sy'];
                        } ?>  -->
                        <h2>
                            <i class="material-icons">description</i> Prelim's Dean's List for S.Y. <b><?php echo $school_year;?>, <?php echo $sem;?></b> . 
                            <a class="pull-right" href="<?php echo base_url(); ?>admin/print_search_prelim_dl/<?php echo $school_year; ?>/<?php echo $sem; ?>" target="__blank">
                    ...
                    <div id="" class="body">
                        <div class="row">
                            <?php foreach($search_prelim_dlist as $key){ 
                                $no_of_subject = 0;
                                $prelim_grade = 0;

                                $this->db->join('subjects', 'grades.sub_id = subjects.sub_id');
                                $this->db->where("stud_id", $key['stud_id']);   
                                $this->db->where("prelim !=", 0); 
                                $this->db->where('sy', $school_year);
                                $this->db->where('semester', $sem);
                                // $this->db->order_by("prelim", "asc");
                                $query = $this->db->get("grades");

                                foreach($query->result() as $rows){//add all data to session    
                                    $prelim_grade += $rows->prelim;
                                    $no_of_subject ++;
                                }   
                                $final_prelim_grade = $prelim_grade / $no_of_subject;
                            ?>
                            <?php if($final_prelim_grade <= 1.74){ ?>
                                <div class="col-sm-6 col-md-3">
                                    <div class="thumbnail text-center">
                                        <div class="caption">
                                            ...
                                            <p class="text-center">
                                                <a href="javascript:void(0);" class="btn btn-primary waves-effect" role="button">AVERAGE: <?php echo substr($final_prelim_grade, 0, 4); ?></a><br/>
                                                <b></b>
                                            </p>
                                        </div>
                                    </div>
